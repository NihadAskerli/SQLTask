...
select * from loancsv;
create table AZN as (select * from loancsv where 2=1);
select * from AZN;
create table dollar as (select * from loancsv where 2=1);
create table tl as (select * from loancsv where 2=1);
create table rubul as (select * from loancsv where 2=1);
insert all 
when currency_id=11 then into AZN
WHEN currency_id=12 THEN into dollar
when currency_id=13 then into tl
when currency_id=14 then into rubul
select * from loancsv;
...
select * from members 
left join member_staging on members.member_id=member_staging.member_id;
